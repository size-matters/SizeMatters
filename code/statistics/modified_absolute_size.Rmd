...
```{r}
grouped_mann_whitney <- function(table) {
  size_x = 0
  size_y = 0
  n = 0
  U_x = 0
  ...
  for (lang in langs){
    ...
    lang_table <- lang_table[!is.na(language),]
    ...
    print("Us are correct", u_x + u_y == n_x * n_y)
    U_x <- U_x + u_x
    U_y <- U_y + u_y
    }
    
    u = min(c(U_x, U_y))
    u_table = qwilcox(0.05, size_x, size_y) - 1
    if (u <= u_table){
      return ("h_1")
    } else{
      return ("h_0")
...
count_mw <- function(table, data){
  ...
  test <- grouped_mann_whitney(table)
  return(test)
}
``` 

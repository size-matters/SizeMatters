...
read_concept <- function(words, concepts, filter_pair){
  ...
  words <- words[!duplicated(words[c("concept","gender", "category", "language")]),]
...
grouped_mann_whitney <- function(table, name, iter) {
   ...
   # Us <- sort(Us, decreasing = FALSE)
    ...
    ggsave(p, 
       filename = sprintf("%s_%s.pdf", name, iter),
       device = "pdf",
...
```{r}
aggregate_results <- function(df,name, iter){
  animal_pvalue <- count_mw(filter(df, category == "animals"), sprintf("animals_%s", name), iter)
  bodypart_pvalue <- count_mw(filter(df, category == "bodyparts"), sprintf("bodyparts_%s", name), iter)
  bird_pvalue <- count_mw(filter(df, category == "birds"), sprintf("birds_%s", name), iter)
  tool_pvalue <- count_mw(filter(df, category == "utensils"), sprintf("utensils_%s", name), iter)
...
for (x in c("A", "B")){
  for (i in c(1000, 5000, 10000)){
    archi_results <- aggregate_results(archi_df, sprintf("archi_%s", x), i)
   # archi_joint_df <- rbind(archi_joint_df, archi_df)
    
    non_archi_results <- aggregate_results(non_archi_df, sprintf("non_archi_%s", x), i)
    #non_archi_joint_df = rbind(non_archi_joint_df, non_archi_df)
    
    write.xlsx(archi_df, file.path(path_to_data, "results",  sprintf("%s_%s_cs joint archi.xlsx", x, as.character(i))))
    write.xlsx(archi_results, file.path(path_to_data, "results", sprintf("%s_%s_cs results archi.xlsx", x, as.character(i))))
    
    write.xlsx(non_archi_df, file.path(path_to_data, "results", sprintf("%s_%s_cs joint non archi.xlsx", x, as.character(i))))
    write.xlsx(non_archi_results, file.path(path_to_data, "results", sprintf("%s_%s_cs results non archi.xlsx", x, as.character(i))))
  }
}
